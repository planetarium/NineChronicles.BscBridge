    ...
    }>
{
  ...
  async notify(data: {
    ...
    if (events.length === 0) {
      await this._monitorStateStore.store("bsc", {
        blockHash,
        txId: null,
      });
    }
    ...
    for (const { returnValues, transactionHash, blockHash } of events) {
      ...
      const requestPlanetName = this._multiPlanetary.getRequestPlanetName(_to);
      ...
      try {
        ...
        const recipient =
          isMultiPlanetRequestType && !isMainPlanetRequest
            ? this._multiPlanetary.getPlanetVaultAddress(requestPlanetName)
            : user9cAddress;

        const memo = isMainPlanetRequest ? transactionHash : user9cAddress;
